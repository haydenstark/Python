...
def center_window(self, w, h):                      #center's tkinter frame based on user's screen
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    x = int((screen_width/2) - (w/2))
    y = int((screen_width/2) - (h/2))
    centerGeo = self.master.geometry('{}x{}+{}+{}'.format(w,h,x,y))
    return centerGeo

def ask_quit(self):             #catches if user clicks 'x' on window
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):     #enforces uses to fill out fields
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
        ...
        messagebox.showerror("Missing Text Error","Please fill out all four fields.")
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 == 0:       #if proposed changes are not already in the database
                response = messagebox.askokcancel("Update Request","The following changes ({}) and ({}) will be implemented for ({}). \n\nProceed with update request?".format(var_phone,var_email,var_value))
                print(response)
                if response:
                    #conn = sqlite3.connect('db_phonebook.db')
                    with conn:
